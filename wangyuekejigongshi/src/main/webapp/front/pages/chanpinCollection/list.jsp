            ...
            <div class="ng-inmod-ini">
                <div class="ng-fault ng-container">
                    <div class="ng-inmod-the ng-container ng-bg-fff">
                        ...
                        <span class="ng-inmod-the-cn" aos="fade-down">产品收藏展示</span>
                    ...
                    <form class="layui-form" lay-filter="myForm">
                          ...
                          <div class="layui-inline" style="margin-bottom: 10px;">
                              ...
                              <div class="layui-input-inline">
                                  <input type="text" name="chanpinName" id="chanpinName" placeholder="产品名称" autocomplete="off" class="layui-input">
                              </div>
                          </div>
                          ...
                          <div class="layui-inline" style="margin-bottom: 10px;">
                              ...
                              <div class="layui-input-inline">
                                  <select name="chanpinTypes" id="chanpinTypes" lay-filter="chanpinTypes">
                                      <option value="">--请选择--</option>
                                      <option v-for="item in chanpinTypesList" :value="item.codeIndex" :key="item.codeIndex">{{ item.indexName }}</option>
                                  </select>
                              </div>
                          </div>
                        ...
                        <ul>
                            <li v-for="(item,index) in dataList" v-bind:key="index">
                                <div class="ng-prslist-doc">
                                    <div class="ng-prslist-img" @click="jump('../chanpin/detail.jsp?id='+item.chanpinId)">
                                        <a href="#" class="ng-prsliik">
                                          <span class="ng-prsliik-ki ng-prsliik-ki_1">
                                            <img class="swiper-item" :src="item.chanpinPhoto?item.chanpinPhoto.split(',')[0]:''" />
                                          </span>
                                    ...
                                    <div class="ng-prslist-tit" @click="jump('../chanpin/detail.jsp?id='+item.chanpinId)"><span>{{item.chanpinName}}</span></div>
                                    <div class="ng-prslist-tit" v-if="item.chanpinNewMoney"><span style="color: red;">{{item.chanpinNewMoney}} RMB</span></div>
                                    <div class="ng-prslist-tit" ><span style="color: red;" @click="deleteChanpinCollection(item.id)">删除 {{item.chanpinName}} 收藏</span></div>
                                </div>
                            </li>
                        </ul>
                    </div>
...
<script>
    var vue = new Vue({
        ...
        filters: {
            //把 chanpinCollectionContent 字段去除无用字符 如果字段长度大于60  就截取到60
            chanpinCollectionContent: function(val) {
                if (val) {
        ...
        methods: {
            ...
            ,deleteChanpinCollection(id){
                layui.layer.confirm('确认要删除这个收藏吗？', {
                    btn : [ '确定', '取消' ]//按钮
                }, function(index) {
                    var paramArray = [];
                    paramArray.push(id);
                    layui.http.requestJson('chanpinCollection/delete', 'post',paramArray, function (res) {
                        if (res.code == 0) {
                            layer.msg('删除成功', {
                                time: 2000,
                                icon: 6
    ...
    layui.use(['layer', 'element', 'carousel', 'laypage','form', 'http', 'jquery', 'laydate'], function() {
        ...
        function pageList() {
            ...
            if (jquery('#chanpinName').val() != null && jquery('#chanpinName').val() != "" && jquery('#chanpinName').val() != "null" ) {
                param['chanpinName'] = jquery('#chanpinName').val();
            }
                     
            var chanpinTypes = $("#chanpinTypes").find("option:selected").val();
            if (chanpinTypes != null && chanpinTypes != "" && chanpinTypes != "null") {
                            param['chanpinTypes'] = chanpinTypes;
                        }
                                                                                
            // 当前表
                                                    // 获取列表数据
            http.request('chanpinCollection/list', 'get', param, function(res) {
...
// 级联表的字典表
       ...
       function chanpinTypesSelect() {
           http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=chanpin_types", 'get', {}, function (res) {
               if(res.code == 0){
                   vue.chanpinTypesList = res.data.list;
               }
           });
       }
